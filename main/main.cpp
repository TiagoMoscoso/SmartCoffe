...
char* rabbitUri = "mqtt://192.168.37.200:1883";
        ...
        if(!rabbitmq.commandQueue.empty())
        {
            ...
            switch (rabbitmq.commandQueue.front())
            {
                case 'L':
                    ...
                    if(warm)
                    {
                        vTaskDelay(CoffeTimes(Large)/(2*portTICK_PERIOD_MS));
                    }
                    else
                    {
                        vTaskDelay(CoffeTimes(Large)/portTICK_PERIOD_MS);
                    }
                    break;
                case 'M':
                    ...
                    if(warm)
                    {
                        vTaskDelay(CoffeTimes(Medium)/(2*portTICK_PERIOD_MS));
                    }
                    else
                    {
                        vTaskDelay(CoffeTimes(Medium)/portTICK_PERIOD_MS);
                    }
                    break;
                case 'S':
                    ...
                    if(warm)
                    {
                        vTaskDelay(CoffeTimes(Small)/(2*portTICK_PERIOD_MS));
                    }
                    else
                    {
                        vTaskDelay(CoffeTimes(Small)/portTICK_PERIOD_MS);
                    }
                    break;
            }
